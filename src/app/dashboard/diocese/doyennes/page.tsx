...
export default function DoyennesPage() {
  ...
  // Charger les doyennes au montage du composant
  useEffect(() => {
    const loadDoyennes = async () => {
      ...
      try {
        ...
        console.error("Erreur lors du chargement des doyennes:", err);
        if (err instanceof AuthenticationError) {
          ...
          setError("Aucune doyenne trouvée pour ce diocèse.");
        } else {
  ...
  // Filtrer les doyennes selon la recherche
  useEffect(() => {
  ...
  // Calculer les doyennes à afficher pour la pagination
  ...
  const exportToExcel = async () => {
    try {
      ...
      const exportData = filteredDoyennes.map((doyenne, index) => ({
        "N°": index + 1,
        "Date de création": formatDate(
          typeof doyenne?.created_at === "number"
            ? new Date(doyenne.created_at).toISOString()
            : doyenne?.created_at
        ),
        "Nom de la Doyenne": doyenne.nom,
      ...
      XLSX.utils.sheet_add_aoa(
        ...
        [
          ...
          [`Nombre total: ${filteredDoyennes.length}`],
          [],
        ],
        { origin: "A1" }
      );
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.text("DOYENNES", 105, 15, { align: "center" });
      ...
      doc.text(`Nombre total: ${filteredDoyennes.length}`, 20, 52);
      ...
      // Données du tableau
      const tableData = filteredDoyennes.map((doyenne, index) => [
        (index + 1).toString(),
        doyenne.nom || "",
        doyenne?.siege?.nom || "",
        doyenne?.siege?.ville || "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Doyennes</h1>
          <p className="text-slate-500">Gérez les doyennes de votre diocèse</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <StatsCard
          title="Total Doyennes"
          value={doyennes.length}
        ...
        <StatsCard
          ...
          value={doyennes.filter((d) => d.doyen_id).length}
          icon={<User size={24} />}
        ...
        <StatsCard
          title="Sièges Actifs"
          value={doyennes.filter((d) => d.siege_id).length}
          icon={<Church size={24} />}
      ...
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              placeholder="Rechercher une doyenne..."
              value={searchQuery}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  disabled={exporting || filteredDoyennes.length === 0}
                >
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">
            Aucune doyenne trouvée
          </h3>
          <p className="text-sm text-slate-500 max-w-md mx-auto mb-4">
            {searchQuery
              ? "Aucune doyenne ne correspond à vos critères de recherche."
              : "Aucune doyenne n'est enregistrée pour ce diocèse."}
          </p>
          {searchQuery && (
            <Button
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-200">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-slate-900">Doyennes</h3>
              <div className="text-sm text-slate-500">
                {filteredDoyennes.length} résultat
                {filteredDoyennes.length > 1 ? "s" : ""}
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-200 flex items-center justify-between">
            <div className="text-sm text-slate-600">
              ...
              {Math.min(currentPage * itemsPerPage, filteredDoyennes.length)}{" "}
              sur {filteredDoyennes.length} résultats
            </div>
