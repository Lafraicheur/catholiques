...
const AjouterNonParoissienForm = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Appel à l'API pour créer le non-paroissien
      const response = await fetch(
        "https://api.cathoconnect.ci/api:HzF8fFua/nonparoissien/creer",
        {
          method: "POST",
          headers: {
      ...
      const newNonParoissien = await response.json();
    ...
    <form onSubmit={handleSubmit}>
      ...
      <DialogFooter className="flex flex-col-reverse sm:flex-row justify-between items-center border-t border-slate-100 pt-4 gap-3 sm:gap-0 mt-2">
        <Button
          ...
          className="border-slate-300 hover:bg-slate-100 hover:text-slate-800 transition-colors w-full sm:w-auto cursor-pointe"
        >
          Annuler
        </Button>
        &nbsp;&nbsp;
...
export default AjouterNonParoissienForm;